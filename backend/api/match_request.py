...
from db.query.match_request import query_match_request_by_id
from enums.match import MatchRequestStatus
from flask import Blueprint, jsonify, request
from flask_jwt_extended import current_user, jwt_required

logger = logging.getLogger(__name__)

match_request = Blueprint("match_request", __name__)


@match_request.route("/match_request/<match_request_id>", methods=["GET"])
@jwt_required()
def get_match_request(match_request_id):

    match_request = query_match_request_by_id(match_request_id)
    return jsonify(match_request.as_dict()), 200


# TODO Cancel any outstanding requests for a user on POST
@match_request.route("/match_request", methods=["POST"])
@jwt_required()
...
@match_request.route("/match_request/status/<match_request_id>", methods=["PUT"])
@jwt_required()
