...
from matchmaker.matching_games.algorithms.util import _delete_pair
from matchmaker.matching_games.exceptions import NoStableMatchingWarning
        ...
        if any(p.prefs == [] for p in players):
            warnings.warn(
                NoStableMatchingWarning(
                    "The following players have emptied their preferences: " f"{[p for p in players if not p.prefs]}"
                )
            )
            break
