...
from matchmaker.matching_games.exceptions import (
...
class BasePlayer:
    ...
    def not_in_preferences_message(self, other):
        ...
        return f"{self} is matched to {other} but they do not appear in their " f"preference list: {self.prefs}."
        ...
        for player in vars(self)[party]:
            ...
            for other in player.prefs:
                if other not in unique_prefs:
                    ...
                    warnings.warn(PreferencesChangedWarning(f"{player} has ranked {other} multiple times."))
        ...
        for player in players:
            for other in player.prefs:
                if other not in others:
                    warnings.warn(
                        PreferencesChangedWarning(f"{player} has ranked a non-{other_party[:-1]}: " f"{other}.")
                    )
                    if self.clean:
